...
if [ -n "$MISSING" ] && command -v dnf &>/dev/null; then
    case " $MISSING " in *" pkexec "*) PKGS="$PKGS polkit" ;; esac
...
if [ -n "$MISSING" ]; then
    ...
    echo "Esim. Fedora: dnf install polkit systemd shadow-utils" >&2
    exit 1
fi
...
if command -v restorecon &>/dev/null; then
    ...
    restorecon /usr/bin/oem-setup-gui 2>/dev/null || true
    restorecon /usr/libexec/oem-setup/oem-setup-helper 2>/dev/null || true
    restorecon /usr/lib/systemd/system/oem-cleanup.service 2>/dev/null || true
fi
